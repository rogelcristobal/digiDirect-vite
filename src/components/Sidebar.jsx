...
const Sidebar = ({ docs, loading }) => {
  return (
    <div className="h-screen bg-[#ffffff] w-fit flex-shrink-0   flex items-start justify-start">
      {/* category */}
      <div className="flex-shrink-0 h-full w-[5rem] bg-[#12334b] thin-right-divider flex flex-col items-center justify-start py-6 ">
        ...
        <div className="text-[1.5rem] text-[#a8a7a7] gap-2 flex items-center justify-around flex-col">
      ...
      <div className="h-full w-[22rem] flex-shrink-0">
        <div className=" mt-32 min-h-[4rem] px-5  w-full">
          <div className="px-0 text-[0.9rem] ml-3 mb-3 text-[#b5b3c1]  font-semibold flex items-center justify-start gap-3">
              ...
              <NavLink key={id} to={`/${item.name}`}>
                {({ isActive }) => (
                  <div
                    key={id}
                    className={
                      `cursor-pointer text-[0.875rem] text-[#1b2838]/80 font-bold py-4 rounded-lg px-4 ${isActive?'bg-[#fafafa]/60 ':'text-[#0a0b3d]/30'} my-2 flex items-center justify-start gap-4  `
                    }
                  >
